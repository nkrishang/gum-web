 ...
 * of a link, standing in until there's a real page to send it to.
 *
 * The dialog is portalled to the body and carries the landing page's palette
 * itself (`.landing-dialog`). Only the trigger belongs to the page around it:
 * `appearance` picks its hover colour.
 */
export function PricingDialog({
  ...
  /** The ground the trigger sits on. */
  appearance?: "dark" | "light";
  trigger?: ReactNode;
  triggerClassName?: string;
  /** An element to use as the trigger instead of the default text button, e.g. a sidebar item. */
  render?: ComponentProps<typeof Dialog.Trigger>["render"];
}) {
  return (
    <Dialog.Root>
      <Dialog.Trigger
      ...
      <Dialog.Portal>
        <Dialog.Backdrop className="landing-dialog-scrim fixed inset-0 z-50 bg-gum-black/40 backdrop-blur-[3px]" />
        <Dialog.Popup className="landing-dialog fixed top-1/2 left-1/2 z-50 w-[calc(100vw-28px)] max-w-[432px] -translate-x-1/2 -translate-y-1/2 rounded-[14px] border border-gum-black bg-gum-white px-6 pt-6 pb-7 text-gum-black outline-none sm:px-7">
